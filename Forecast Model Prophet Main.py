...
def generate_date_from_week(row):
    week_num = int(row['week'][1:])      # e.g. 'W53' → 53
    year     = int(row['year'])

    # Determine the real last ISO week for this year:
    # Dec 28 is guaranteed to lie in the year's last ISO week
    last_iso_week = datetime(year, 12, 28).isocalendar()[1]

    # Clamp out-of-range week numbers down to last_iso_week
    if week_num > last_iso_week:
...
def calculate_summary_statistics(ts_data, forecast_df, horizon):
    """
    Returns:
      summary_stats: dict of min/max/mean/…
      total16, total8, total4: ints
      max_forecast, min_forecast: ints
      max_week, min_week: timestamps or None
    """
    # --- historical stats ---
    summary_stats = {
        "min": ts_data["y"].min(),
    ...
    # --- forecast totals ---
    my = forecast_df.get("MyForecast", pd.Series(dtype=float)).fillna(0).astype(int)
    n = len(my)
    total16 = int(my.iloc[:16].sum()) if n>=16 else int(my.sum())
    total8  = int(my.iloc[:8].sum())  if n>=8  else int(my.sum())
    total4  = int(my.iloc[:4].sum())  if n>=4  else int(my.sum())

    # --- max/min values ---
    max_fc = int(my.max()) if n>0 else 0
    min_fc = int(my.min()) if n>0 else 0

    # --- safe lookup of the week for the max/min forecasts ---
    def safe_week(idx_fn):
        if "ds" not in forecast_df.columns or n == 0:
            return None
        try:
            idx = idx_fn()
            return forecast_df.at[idx, "ds"]
        except Exception:
            return None

    max_week = safe_week(lambda: my.idxmax())
    min_week = safe_week(lambda: my.idxmin())

    return summary_stats, total16, total8, total4, max_fc, min_fc, max_week, min_week
        ...
        if non_nan_count_with_po < 10:
            ...
            if model_type_with_po == "SARIMA":
                ...
                try:
                    ...
                    # Fallback: use Amazon Mean forecast as fallback.
                    ...
                    fallback_dates = pd.date_range(start=ts_data_with_po['ds'].iloc[-1] + pd.Timedelta(weeks=1),
                                                periods=horizon, freq='W-SUN')
                    final_forecast_df_with_po = pd.DataFrame({'ds': fallback_dates, 'MyForecast': amazon_mean_series})
                ...
                # --- Blend with Amazon forecasts using RMSE-based weights (if possible) ---
                if forecast_data:
                    ...
                    # Only apply dynamic weighting if there are enough overlapping actual values.
                ...
                # --- Adjust forecast if the overall forecast mean is too high relative to past 8 weeks ---
                ...
                for irow in range(len(comparison_with_po)):
                    ...
                    reorder_urg_list.append("Urgent" if coverage < 1.0 else "Normal")
                    stockout_risk_list.append("High" if coverage < 0.5 else "Low")
