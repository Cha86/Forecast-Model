...
def week_label_to_date(week_label, base_year):
    ...
    Assumes first week of base_year starts on January 1st.
    """
...
def save_summary_to_excel(comparison,
                          ...
                          base_year=2025):
    """
    Save summary statistics and forecast data to an Excel file.
    """
    ...
    if 'Week' not in comparison.columns:
        ...
        comparison['Week'] = ['W' + str(i+1).zfill(2) for i in range(len(comparison))]
...
def save_forecast_to_excel(output_path, consolidated_data, missing_asin_data, base_year=2025):
    ...
    for asin, df in consolidated_data.items():
        if df.empty:
            summary_data["ASIN"].append(asin)
            summary_data["Product Title"].append("No data")
            summary_data["4 Week Forecast"].append(None)
            summary_data["8 Week Forecast"].append(None)
            summary_data["16 Week Forecast"].append(None)
            continue

        product_title = df['Product Title'].iloc[0] if 'Product Title' in df.columns else ''
        four_wk_val = df['MyForecast'][:4].sum() if len(df) >= 4 else df['MyForecast'].sum()
        eight_wk_val = df['MyForecast'][:8].sum() if len(df) >= 8 else df['MyForecast'].sum()
        sixteen_wk_val = df['MyForecast'][:16].sum() if len(df) >= 16 else df['MyForecast'].sum()

        summary_data["ASIN"].append(asin)
        summary_data["Product Title"].append(product_title)
        summary_data["4 Week Forecast"].append(int(four_wk_val))
        summary_data["8 Week Forecast"].append(int(eight_wk_val))
        summary_data["16 Week Forecast"].append(int(sixteen_wk_val))
...
def save_4_8_16_forecast_summary(consolidated_data, output_folder):
    ...
    for asin, df in consolidated_data.items():
        ...
        four_wk_val = df['Cumulative Forecast'].iloc[3] if len(df) > 3 else df['Cumulative Forecast'].sum()
        eight_wk_val = df['Cumulative Forecast'].iloc[7] if len(df) > 7 else df['Cumulative Forecast'].sum()
        sixteen_wk_val = df['Cumulative Forecast'].iloc[15] if len(df) > 15 else df['Cumulative Forecast'].sum()
